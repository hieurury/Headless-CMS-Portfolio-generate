        ...
        children: [
          {
            id: 'hero-heading-001',
            type: 'heading',
            props: { text: 'Hello World', level: 'h1', size: '5xl' },
            children: [],
          },
          {
            id: 'hero-desc-001',
            type: 'description',
            props: { text: 'I build things.', size: 'lg' },
            children: [],
          },
        ],
      },
...
console.log('\n[TEST 1] DELETE: Xoá "footer-001"');
{
  const result = applyModification(mockLayout, {
    type: AI_ACTION.DELETE,
    targetId: 'footer-001',
  });
  assert(result.length === 2, 'Top-level section count giảm từ 3 → 2');
  assert(
    !findById(result, 'footer-001'),
    '"footer-001" không còn tồn tại trong cây',
  );
...
console.log('\n[TEST 2] DELETE nested: Xoá "link-about" ở sâu trong cây');
{
  ...
  assert(
    flexNode?.children.length === 1,
    '"nav-links-001" chỉ còn 1 child (link-contact)',
  );
...
console.log('\n[TEST 3] UPDATE: Đổi text "hero-heading-001"');
{
  const result = applyModification(mockLayout, {
    ...
    newNode: {
      id: 'hero-heading-001',
      type: 'heading',
      props: { text: 'I Build Digital Experiences', level: 'h1', size: '5xl' },
      children: [],
    },
  });
  const updated = findById(result, 'hero-heading-001');
  assert(
    updated?.props.text === 'I Build Digital Experiences',
...
console.log('\n[TEST 4] ADD_CHILD: Thêm badge vào trong "nav-links-001"');
{
  const result = applyModification(mockLayout, {
    ...
    newNode: {
      id: 'btn-hire',
      type: 'button',
      props: { label: 'Hire Me', href: '#contact', variant: 'primary' },
      children: [],
    },
  });
  const flexNode = findById(result, 'nav-links-001');
  assert(
    flexNode?.children.length === 3,
    '"nav-links-001" có 3 children (thêm 1)',
  );
  const newBtn = flexNode?.children.find((c: any) => c.type === 'button');
  assert(
    newBtn?.props.label === 'Hire Me',
    'Button "Hire Me" được thêm vào cuối',
  );
}
...
);
{
  const result = applyModifications(mockLayout, [
    { type: AI_ACTION.DELETE, targetId: 'footer-001' },
    {
      ...
      newNode: {
        id: 'hero-heading-001',
        type: 'heading',
        props: { text: 'Pipeline Works!', level: 'h1' },
        children: [],
      },
    },
    {
      ...
      newNode: {
        id: 'badge-new',
        type: 'badge',
        props: { text: 'React', variant: 'subtle', color: 'sky' },
        children: [],
      },
    },
  ]);
  ...
  assert(
    heading?.props.text === 'Pipeline Works!',
    'Heading đã được update đúng',
  );
  const flexNode = findById(result, 'nav-links-001');
  assert(
    flexNode?.children.length === 3,
    'nav-links-001 có 3 children sau khi add badge',
  );
}
...
console.log('\n[TEST 8] Immutability: Dữ liệu gốc không bị mutate');
{
  ...
  assert(
    ...
    `Cây gốc vẫn có ${originalCount} nodes, không bị mutate`,
  );
}
